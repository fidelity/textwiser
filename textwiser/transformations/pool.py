...
class _PoolTransformation(_BaseTransformation):
    ...
    def _pool(self, x):
        if self.pool_option == PoolOptions.max:
            return torch.max(x, dim=0)[0]
        if self.pool_option == PoolOptions.min:
            return torch.min(x, dim=0)[0]
        if self.pool_option == PoolOptions.mean:
            return torch.mean(x, dim=0)
        if self.pool_option == PoolOptions.first:
            return x[0]
        if self.pool_option == PoolOptions.last:
            return x[-1]
